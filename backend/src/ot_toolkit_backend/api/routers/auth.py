...
def register(payload: RegisterRequest, store: DatabaseStore = Depends(get_store)) -> UserOut:
    user = store.add_user(payload.username, hash_password(payload.password))
    return UserOut(username=user.username)


@router.post("/token", response_model=TokenOut)
def login(payload: LoginRequest, store: DatabaseStore = Depends(get_store)) -> TokenOut:
