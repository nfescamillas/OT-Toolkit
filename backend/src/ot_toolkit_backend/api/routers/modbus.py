...
def convert_address(payload: ModbusAddressRequest, store: InMemoryStore = Depends(get_store)):
...
def decode_registers(payload: RegisterDecodeRequest, store: InMemoryStore = Depends(get_store)):
    return safe_float_fields(store.service.modbus_registers(payload.registers))


@router.post("/crc", response_model=ModbusCrcOut)
def calculate_crc(payload: HexBytesRequest, store: InMemoryStore = Depends(get_store)):
    return store.service.modbus_crc(payload.hex)


@router.post("/frame/decode", response_model=FrameDecodeOut)
def decode_frame(payload: HexBytesRequest, store: InMemoryStore = Depends(get_store)):
