...
def calculate_subnet(payload: SubnetRequest, store: DatabaseStore = Depends(get_store)):
    return store.service.subnet(payload.ip, payload.mask_or_cidr)


@router.post("/same-subnet", response_model=SameSubnetOut)
def same_subnet(payload: SameSubnetRequest, store: DatabaseStore = Depends(get_store)):
...
def validate_ips(payload: IpValidationRequest, store: DatabaseStore = Depends(get_store)):
    return store.service.validate_ips(payload.values, payload.mask_or_cidr)


@router.post("/commands", response_model=CommandOut)
def commands(payload: CommandRequest, store: DatabaseStore = Depends(get_store)):
