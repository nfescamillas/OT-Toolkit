...
def get_technology(technology_id: str, store: DatabaseStore = Depends(get_store)):
    return store.service.get_technology(technology_id)


@router.get("/categories", response_model=list[str])
def list_categories(store: DatabaseStore = Depends(get_store)):
    return store.service.list_categories()


@router.get("/ports", response_model=list[PortOut])
def list_ports(q: str = Query(default=""), store: DatabaseStore = Depends(get_store)):
    return store.service.list_ports(q)


@router.get("/glossary", response_model=list[GlossaryOut])
def list_glossary(q: str = Query(default=""), store: DatabaseStore = Depends(get_store)):
    return store.service.list_glossary(q)


@router.get("/troubleshooting", response_model=dict[str, list[str]])
def list_troubleshooting(store: DatabaseStore = Depends(get_store)):
