...
def search(q: str = Query(min_length=1), store: InMemoryStore = Depends(get_store)):
    return store.service.search(q)


@router.post("/comparisons", response_model=ComparisonOut)
def compare(payload: ComparisonRequest, store: InMemoryStore = Depends(get_store)):
...
def hierarchy(technology_id: str, store: InMemoryStore = Depends(get_store)):
